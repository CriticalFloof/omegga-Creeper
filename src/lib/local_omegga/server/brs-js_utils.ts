...
function write_i32(num: number, littleEndian = true): Uint8Array {
    const data = new Uint8Array([num & 255, (num >> 8) & 255, (num >> 16) & 255, (num >> 24) & 255]);
...
function write_string(str: string): Uint8Array {
    if (isASCII(str)) {
        ...
        return concat(
            ...
            new Uint8Array(str.split("").flatMap((s) => [s.charCodeAt(0) & 0xff, s.charCodeAt(0) >> 8]))
            // new Uint8Array([0]) // Null terminator
        );
    }
}
...
export function subarray(data: og.Bytes, len: number, isCopy = false): Uint8Array {
    ...
    const chunk = bytes[isCopy ? "slice" : "subarray"](bytes.brsOffset, bytes.brsOffset + len);
    bytes.brsOffset += len;
    return chunk;
}

function write_uuid(uuid: og.Uuid) {
    return concat(...chunk(uuidParse(uuid), 4).map(([a, b, c, d]) => new Uint8Array([d, c, b, a])));
}
...
export const bgra = ([b, g, r, a]: number[]): [number, number, number, number] => [r, g, b, a];
...
export class BitWriter {
    ...
    unreal(type: string, value: og.UnrealType) {
        switch (type) {
            ...
            case "String":
                if (typeof value !== "string") {
                    throw new Error(`writing unreal type String, did not receive string (${value})`);
                }
                this.string(value);
                return;
            case "Object":
